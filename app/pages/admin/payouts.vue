<template>
  <div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      <div>
        <h1 class="text-2xl font-black text-white">Finance & Payouts</h1>
        <p class="text-xs text-slate-400 mt-1">Process partner commission withdrawals and track payout history</p>
      </div>
      <button 
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
      <div class="bg-[#0b1320] border border-slate-800 p-4 rounded-2xl">
        <p class="text-slate-400 text-[11px] font-medium">Pending Payouts</p>
        <h2 class="text-xl font-black text-amber-400 mt-1">{{ totalPending.toLocaleString() }} ETB</h2>
        <span class="text-[10px] text-slate-500">{{ pendingPayouts.length }} Requests waiting</span>
      </div>

      <div class="bg-[#0b1320] border border-slate-800 p-4 rounded-2xl">
        <p class="text-slate-400 text-[11px] font-medium">Total Paid Out</p>
        <h2 class="text-xl font-black text-emerald-400 mt-1">{{ totalCompleted.toLocaleString() }} ETB</h2>
        <span class="text-[10px] text-slate-500">{{ completedPayouts.length }} Payments processed</span>
      </div>

      <div class="bg-[#0b1320] border border-slate-800 p-4 rounded-2xl">
        <p class="text-slate-400 text-[11px] font-medium">Platform Total Commission</p>
        <h2 class="text-xl font-black text-white mt-1">{{ (totalPending + totalCompleted).toLocaleString() }} ETB</h2>
        <span class="text-[10px] text-emerald-500">Live Balance Track</span>
      </div>
    </div>
    ...
    <div class="space-y-6">
      ...
      <div class="bg-[#0b1320] border border-slate-800 rounded-2xl p-5 space-y-4">
        <div class="flex items-center justify-between border-b border-slate-800 pb-3">
          <h2 class="text-sm font-bold text-amber-400">Pending Withdrawal Requests</h2>
          <span class="text-xs bg-amber-500/10 text-amber-400 border border-amber-500/20 px-2.5 py-0.5 rounded-full font-bold">
        ...
        <div v-if="pendingPayouts.length > 0" class="space-y-3">
          <div 
            ...
            :key="p.id" 
            class="flex flex-col sm:flex-row sm:items-center justify-between bg-[#070c14] p-4 rounded-xl border border-slate-800/80 gap-3"
          >
            <div>
              <p class="text-xs font-bold text-white">{{ p.venue }}</p>
              <p class="text-[10px] text-slate-400">{{ p.bank }} - <span class="text-slate-300 font-mono">{{ p.account }}</span></p>
              <p class="text-[10px] text-slate-500 mt-0.5">Requested: {{ p.date }}</p>
            </div>

            <div class="flex items-center justify-between sm:justify-end gap-4">
              <div class="text-left sm:text-right">
                <p class="text-xs font-bold text-emerald-400">{{ p.amount.toLocaleString() }} ETB</p>
              </div>
              
              <div class="flex items-center gap-2">
                <button 
                  @click="confirmPayout(p)" 
                  class="text-[10px] bg-emerald-600 hover:bg-emerald-500 text-white px-3 py-1.5 rounded-lg font-bold transition"
                >
                  Process Payment
                </button>
                <button 
                  @click="rejectPayout(p.id)" 
                  class="text-[10px] bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 border border-rose-500/30 px-3 py-1.5 rounded-lg font-bold transition"
                >
                  Reject
                </button>
              </div>
      ...
      <div class="bg-[#0b1320] border border-slate-800 rounded-2xl p-5 space-y-4">
        <div class="flex items-center justify-between border-b border-slate-800 pb-3">
          <h2 class="text-sm font-bold text-emerald-400">Processed Payment History</h2>
        ...
        <div v-if="completedPayouts.length > 0" class="overflow-x-auto">
          <table class="w-full text-left text-xs text-slate-300">
            ...
            <tbody class="divide-y divide-slate-800/60">
              <tr v-for="h in completedPayouts" :key="h.id" class="hover:bg-slate-800/20">
                <td class="p-3 font-bold text-white">{{ h.venue }}</td>
                <td class="p-3 text-slate-400">{{ h.bank }} ({{ h.account }})</td>
                <td class="p-3 font-bold text-emerald-400">{{ h.amount.toLocaleString() }} ETB</td>
    ...
    <div v-if="showModal" class="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 z-50">
      <div class="bg-[#0b1320] border border-slate-800 w-full max-w-md rounded-2xl p-6 space-y-4">
        <h2 class="text-lg font-bold text-white">Create Payout Request</h2>

        <form @submit.prevent="saveNewRequest" class="space-y-4">
          <div>
            <label class="block text-xs font-bold text-slate-400 mb-1">Venue Name</label>
            <input 
              v-model="form.venue" 
              type="text" 
              required 
          ...
          <div>
            <label class="block text-xs font-bold text-slate-400 mb-1">Bank Name</label>
            <select 
              v-model="form.bank" 
              required
              class="w-full bg-[#070c14] border border-slate-800 text-white text-xs rounded-xl p-3 focus:outline-none focus:border-emerald-500"
          ...
          <div>
            ...
            <input 
              ...
              required 
              placeholder="1000..."
              class="w-full bg-[#070c14] border border-slate-800 text-white text-xs rounded-xl p-3 focus:outline-none focus:border-emerald-500 font-mono"
            />
          </div>

          <div>
            <label class="block text-xs font-bold text-slate-400 mb-1">Amount (ETB)</label>
            <input 
